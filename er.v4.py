...
def unpack_startsends(chrom): 
	...
	with open(cov_file) as f:
		...
		for line in f: 
			...
			if chrom not in out:
				out[chrom] = {(bc,strand):{umi:[(starts, ends), b"", b""]}} # dict()
			elif (bc,strand) not in out[chrom]:
				out[chrom][bc,strand] = {umi:[(starts, ends), b"", b""]} #(starts, ends)
			else:
				out[chrom][bc,strand][umi] = [(starts, ends), b"", b""] #(starts, ends)
...
def pileup(pos, cigar, seq):
	...
	out[chrom][bc,strand][umi][2] += bytes("".join([gx, ",", gn, " "]), encoding='utf-8') #(gx,gn)
...
def across_chrom(chrom):
	global out
	for bc_strand in list(out[chrom].keys()):
		if bc_strand[0] not in cell_cov:
			cell_cov[bc_strand[0]]=[0,0,0]
		gxgns = list()
		
		for umi in list(out[chrom][bc_strand].keys()):
			gx_gn = str(out[chrom][bc_strand][umi][2], encoding="utf-8").split(" ")
			gx_gn = bytes(max(gx_gn,key=gx_gn.count), encoding='utf-8')
			#gx,gn = gx_gn.split(",")
			if (bc_strand, gx_gn) not in cell_cov:
				cell_cov[(bc_strand, gx_gn)]=[0,0,0]
			...
			while True: 
				...
				bc_strand_gxgn = (bc_strand,gx_gn)
				if gx_gn not in gxgns:
					gxgns.append(gx_gn)
				#print(start, out[chrom][bc_strand].get(start,[]), ref)
				out[chrom][bc_strand_gxgn] = out[chrom].get(bc_strand_gxgn,{})
				out[chrom][bc_strand_gxgn][start] = out[chrom][bc_strand_gxgn].get(start,[]) + [ref] #.append(ref)
				#print(out[chrom][bc_strand][start])
				if not dups_dict:
					#out[chrom][bc_strand][start] = out[chrom][bc_strand].get(start,[]) + [(gx,gn)]
		...
		for gx_gn in gxgns:
			bc_strand_gxgn = (bc_strand, gx_gn)
			for start in sorted(out[chrom][bc_strand_gxgn].keys()):
				if start not in out[chrom][bc_strand_gxgn]:
					continue
				pos = start-1
				seqs=[]
				ref = ""
				...
				while True:
					pos += 1
					if pos in out[chrom][bc_strand_gxgn]:
						seqs.append(out[chrom][bc_strand_gxgn][pos])
						seqs[-1].append(0) # append counter
						out[chrom][bc_strand_gxgn].pop(pos)
					nts_at_pos = list()
					...
					if len(nt_types_at_pos) == 1:
						...
						cell_cov[bc_strand_gxgn][0] += len(nts_at_pos)
						#cell_cov[bc_strand[0]][1] += 1
						cell_cov[bc_strand_gxgn][1] += 1
					else:
						b = {nt_type:nts_at_pos.count(nt_type) for nt_type in nt_types_at_pos}
						#ref=''.join([ref,"(",*[str(v)+k for k,v in b.items()], ")" ])
						pcr_ers=list()
						...
						if b and max(b.values())>=cov_min-1:
							...
							if (len(b)==2 and min(b.values())==1): # or (print_var and pos==print_var[1] and chrom==print_var[0])
								print(bc_strand[0][5:], chrom, bc_strand[1], str(gx_gn, encoding="utf-8"), pos, *[k for k,v in sorted(b.items(), key=lambda item: item[1], reverse=True)], ''.join([str(v)+k for k,v in b.items()]), ''.join(pcr_ers)) # ref.rstrip("(")[:-1]
								#cell_cov[bc_strand[0]][0] += sum(b.values())
								#cell_cov[bc_strand[0]][1] += 1
								#cell_cov[bc_strand[0]][2]+=1
								cell_cov[bc_strand_gxgn][0] += sum(b.values())
								cell_cov[bc_strand_gxgn][1] += 1
								cell_cov[bc_strand_gxgn][2]+=1
...
for line in sys.stdin:
	...
	if not line[-3].startswith("GN"):
		if not line[-3].startswith("GX"):
			gn=""; gx=""
		else:
			gn=""; gx=line[-3][5:]
	else:
		if not line[-4].startswith("GX"):
			gn=line[-3][5:]; gx=""
		else:
			gn=line[-3][5:]; gx=line[-4][5:]
	#if gene
	# put bc,strand,genes[gx],genes[gn] ; no, bc these lines would be sabed one time either ingenes or here, so just bc,strand,gx,gn
	# but do we need both gx and gn? check if we have lines where we have gn but not gx
	...
	if (chrom not in out) or ((bc,strand) not in out[chrom]) or (umi not in out[chrom][bc,strand]):
		...
		#	print("not in out")
		continue
	#if out[chrom][bc,strand][umi][2] and not out[chrom][bc,strand][umi][2] == (gx,gn):
	#	del [chrom][bc,strand][umi] # but what if gx not assigned for some duplicates? but 
...
with open(address_er, "w") as f:
	for bc_strand_gxgn in cell_cov:
		if not cell_cov[bc_strand_gxgn][1]: # if cov_per_pos == 0
			continue
		print(bc_strand_gxgn[0][0][5:],
			str(bc_strand_gxgn[1], encoding="utf-8"),
			round(cell_cov[bc_strand_gxgn][2]/cell_cov[bc_strand_gxgn][1], 8), 
			*cell_cov[bc_strand_gxgn][::-1],
			file=f)
